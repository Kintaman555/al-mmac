...
/*
  ...
  for(ld = list_head(distribution_list); ld != NULL; ld = list_item_next(ld)){
	  if(ld->distributions[slot].successful_prob > seed){
		  rimeaddr_copy(&strategy[slot], &ld->parent);
		  break;
	  }
  }
  return;*/
...
/*---------------------------------------------------------------------------*/
static void set_transmit_mode(rimeaddr_t *to)
{
	mode = TRANSMIT_MODE;
...
PROCESS_THREAD(dutycycle_process, ev, data)
{
  ...
  if(!learning_disable_beacon()){ //disable beacon when learning
      ...
      if(incoming) {
    	...
    	/* send data preamble */
    	  ...
    	  if(we_are_learning != NOT_YET_LEARNING){ //we must change receiver address in MAC header
		    rimeaddr_copy(&(hdr->receiver), &strategy[slot - 1]);
		  }
    	  /* send the packet */
		  ...
		  while (!got_ack && RTIMER_CLOCK_LT(RTIMER_NOW(), t0 + ACK_WAIT)){
			...
			if (len > 0){
			  if (ack.type.ptype == TYPE_ACK && rimeaddr_cmp(&(ack.receiver),&rimeaddr_node_addr) &&
					  rimeaddr_cmp(&(ack.sender), &(hdr->receiver))){
				off();
				got_ack = 1;
	...
	off();
	//printf("slot %d: %d \n", slot, get_tx_channel(RTIMER_TIME(&rt), &rimeaddr_node_addr));
  }
  ...
  case TYPE_DATA: {
    ...
    if(!is_broadcast){
      ...
      /* for me, so send ACK, then notify upper layer */
        ...
        ack.type.seq = 1;
        rimeaddr_copy(&(ack.sender),&rimeaddr_node_addr);
        rimeaddr_copy(&(ack.receiver), &(hdr->sender));
...
/*if(we_are_learning == STILL_LEARNING && rimeaddr_cmp(&rimeaddr_null, packetbuf_addr(PACKETBUF_ADDR_RECEIVER))){
	return;
} else {*/
